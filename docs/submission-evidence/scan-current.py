...
root=Path(r'D:\Lean\jsp-000513-cleanroom\repo');out=root/'docs/submission-evidence'
...
for f in files:
 p=root/f
 if p.suffix not in ['.lean','.ps1','.toml'] or f.startswith('docs/'):continue
...
print('Current production source/config/script files scanned:',len(result));print('Hits:',json.dumps([x for x in result if x['hits']],ensure_ascii=False))
